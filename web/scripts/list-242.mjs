// 列出 242 個「可全玩」本質設計（除去 R/Y/B 顏色置換 + 鏡射手性），存成研究用資料。
// 從 web/ 執行：node scripts/list-242.mjs  → 寫出 docs/data/fully-playable-242.{json,csv}
...
function mirrorIndex(quad){const m=quad.map(applyM);let eA,eB,eC,eD;
  ...
  return p.off+eA.idx+p.a*eB.idx+p.a*p.b*eC.idx+p.a*p.b*p.c*eD.idx;}
const o0=301545,mo0=mirrorIndex(unrank(o0));
console.log('\n[鏡射] M 是對合？',mirrorIndex(unrank(mo0))===o0,' 原始#301546 鏡射→#'+(mo0+1)+' 仍可全玩？',fullSet.has(mo0));
let inSet=0;for(const N of full){const M=mirrorIndex(unrank(N));if(M>=0&&fullSet.has(M))inSet++;}
console.log(`[鏡射] 2904 組的鏡射仍落在可全玩集合：${inSet}/${full.length}`);
if(inSet===full.length){
  ...
  console.log(`[鏡射+色置換] 合併軌道數：${orb2}`);
  console.log(`[鏡射+色置換] 軌道大小分布：`,sz2,'（6=自身鏡射對稱/achiral，12=手性對被合併）');
}

// —— 列出 242 個本質設計並存檔 ——
const RYB={2:'R',1:'Y',0:'B'};
const facesOf=tri=>{const out=[];for(let f=0;f<6;f++){const code=cf(tri[f*4],tri[f*4+1],tri[f*4+2],tri[f*4+3]);out.push([...String(code).padStart(4,'0')].map(d=>RYB[+d]).join(''));}return out;};
const seenL=new Set(),recs=[];
for(const N of full){if(seenL.has(N))continue;
  const stack=[N],orb=new Set([N]);
  while(stack.length){const x=stack.pop(),q=unrank(x);for(const y of [mirrorIndex(q),...perm3.map(s=>indexAfter(s,q))])if(y>=0&&!orb.has(y)){orb.add(y);stack.push(y);}}
  for(const x of orb)seenL.add(x);
  const members=[...orb].sort((a,b)=>a-b),rep=members[0],q=unrank(rep),rk=['jia','yi','bing','ding'],cubes={},faces={};
  q.forEach((tri,i)=>{cubes[rk[i]]=tri.slice();faces[rk[i]]=facesOf(tri);});
  recs.push({orbit:0,repIndex:rep+1,mirrorIndex:mirrorIndex(q)+1,members:members.map(x=>x+1),cubes,faces});
}
recs.sort((a,b)=>a.repIndex-b.repIndex);recs.forEach((r,i)=>r.orbit=i+1);
fs.mkdirSync('../docs/data',{recursive:true});
fs.writeFileSync('../docs/data/fully-playable-242.json',JSON.stringify({total:recs.length,
  note:'515,780 種「24 面全不同」設計中，能拼出全部 13 個圖形（可全玩）的本質設計，已除去 R/Y/B 顏色置換與鏡射手性（每個本質設計有 12 個變體：6 色 × 2 手性）。repIndex 為「排列研究」套組編號(1-based)，可直接在網頁套組設定輸入。cubes 為甲乙丙丁的 24 三角形 base 塗色(0藍/1黃/2紅)，faces 為各 6 面的旋轉標準型(R/Y/B)。',
  designs:recs},null,1));
const csv=['orbit,repIndex,mirrorIndex,jia,yi,bing,ding',
  ...recs.map(r=>`${r.orbit},${r.repIndex},${r.mirrorIndex},${r.faces.jia.join(' ')},${r.faces.yi.join(' ')},${r.faces.bing.join(' ')},${r.faces.ding.join(' ')}`)].join('\n');
fs.writeFileSync('../docs/data/fully-playable-242.csv',csv+'\n');
console.log(`\n已輸出 ${recs.length} 個本質設計 → docs/data/fully-playable-242.{json,csv}`);
console.log('前 6 個（套組編號 / 甲乙丙丁各 6 面）：');
for(const r of recs.slice(0,6))console.log(`  #${String(r.repIndex).padStart(6)}  甲[${r.faces.jia.join(' ')}] 乙[${r.faces.yi.join(' ')}] 丙[${r.faces.bing.join(' ')}] 丁[${r.faces.ding.join(' ')}]`);
