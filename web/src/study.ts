...
export function renderStudy(host: HTMLElement, hooks?: CubeSetHooks) {
  ...
  host.innerHTML = `
    <div class="study-wrap">
      ...
      <section class="study-card">
        <div class="study-card-h">再延伸：那 13 個圖形是「全部」嗎？</div>
        <p>
          四顆方塊能拼出的「形狀」其實不只 13 種。若採「面接觸或邊接觸都算相連、鏡像視為不同」的標準，
          共有 <b>88 種形狀</b>（22 平面 ＋ 66 立體；其中只有 8 種是純面相連的 tetracube）。
          官方那 13 個只是其中<b>人工挑選</b>的一小撮。
        </p>
        <div class="funnel">
          <div class="funnel-step" style="--w:100%"><b>88</b><span>四方塊全部形狀（面+邊接觸、鏡像分開）</span></div>
          <div class="funnel-arrow">↓ 拿原始 2011 套組去拼（規則：外觀外部相鄰邊同色）</div>
          <div class="funnel-step" style="--w:64%"><b>87 / 88</b><span>原始套組解得開——唯一卡住的是「對角雙柱」</span></div>
        </div>
        <p>
          換句話說，13 個圖形<b>不是</b>為了「這套方塊解得開」而挑的——它幾乎什麼形狀都拼得出來，
          13 純粹是當年的<b>展示性挑選</b>。那唯一無解的「對角雙柱」是 2×2×2 盒裡兩條對角直立方塊、
          只靠一條邊相連，且是<b>自鏡像</b>（鏡射也救不了）。
        </p>
        <p>把 88 形狀套到全部 242 組，整批設計被這唯一形狀切成兩半：</p>
        <div class="study-split">
          <div class="study-split-box ok"><b>154 組</b><span>解開全部 88 種</span></div>
          <div class="study-split-box bad"><b>88 組</b><span>各差 1 種，且<b>全卡在對角雙柱</b></span></div>
        </div>
        <p class="study-note">其餘 87 種形狀，242 組無一例外全部解得開——88 種裡只有「對角雙柱」會篩人。
          原始 2011 那組正落在 87/88 這群。</p>
      </section>

      <section class="study-card" id="shapes-section"></section>

      <section class="study-card">
        <div class="study-card-h">那 154 組裡，哪一組最值得當預設？ <span class="study-sub">沒有唯一答案，分四種性格</span></div>
        <p class="study-note" style="margin-top:0 !important">
          提醒：用「解數總和」比精瘦是爛指標（幾乎全被圖形四灌爆）。換成幾何平均難度、唯一解關卡、
          每顆方塊內的顏色均勻度，冠軍各不相同。全域 RYB 恆為 32/32/32（規則強制），不能拿來選。
        </p>
        <div class="preset-list">
          <div class="preset-item">
            <div class="preset-head"><b>① 傳承／原始</b><code>#301546</code></div>
            <div class="preset-tags"><span>難度 100.9</span><span>唯一解關 0</span><span>色偏 44</span></div>
            <p>13 官方完美，但無唯一解關、解不開對角雙柱。</p>
          </div>
          <div class="preset-item">
            <div class="preset-head"><b>② 綜合全才</b><code class="accent">#10594</code></div>
            <div class="preset-tags"><span>難度 91.0</span><span>唯一解關 1</span><span>色偏 40</span></div>
            <p>有唯一解關 ＋ 難度跨度廣 ＋ 88 全解，綜合最佳。</p>
          </div>
          <div class="preset-item">
            <div class="preset-head"><b>③ 整體最難</b><code class="accent">#150693</code></div>
            <div class="preset-tags"><span>難度 80.6（最難）</span><span>唯一解關 0</span><span>色偏 48</span></div>
            <p>每關最緊、解法最少，挑戰性最高。</p>
          </div>
          <div class="preset-item">
            <div class="preset-head"><b>④ 視覺最均</b><code class="accent">#4223</code></div>
            <div class="preset-tags"><span>難度 159.2（最鬆）</span><span>唯一解關 0</span><span>色偏 16（最均）</span></div>
            <p>每顆都接近 8/8/8、最繽紛（也因此最好解）。</p>
          </div>
        </div>
        <p class="study-note">「難度」為幾何平均解數（越低越難）；「色偏」為每顆方塊離 8/8/8 的偏差和（越低越均）。
          視覺均勻 ⇄ 難度是反向的：越繽紛越好解。預設仍保留①；②③④可在上方
          <b>「方塊套組設定 → 用編號選 → 推薦套組」</b>一鍵切換、預覽、套用到試玩。</p>
      </section>
  ...
  mountShapes(host.querySelector<HTMLElement>('#shapes-section')!);
...
class ShapeViewer3D {
  ...
  constructor(private host: HTMLElement) {
    ...
    this.scene.add(new THREE.AmbientLight(0xffffff, 0.82));
    const key = new THREE.DirectionalLight(0xffffff, 0.92); key.position.set(5, 8, 6); this.scene.add(key);
    const fill = new THREE.DirectionalLight(0x9db4ff, 0.32); fill.position.set(-5, -2, -4); this.scene.add(fill);
    this.scene.add(this.group);
    host.appendChild(this.renderer.domElement);
    this.controls = new OrbitControls(this.camera, this.renderer.domElement);
  ...
  setShape(cells: Cell[]) {
    while (this.group.children.length) { const c = this.group.children[0]; this.group.remove(c); }
    const ctr = (i: number) => (Math.min(...cells.map((c) => c[i])) + Math.max(...cells.map((c) => c[i]))) / 2;
    const cx = ctr(0), cy = ctr(1), cz = ctr(2);
    for (const [x, y, z] of cells) {
      const geo = new THREE.BoxGeometry(1, 1, 1); // 緊貼無縫，靠邊線分界
      const mat = new THREE.MeshLambertMaterial({ color: 0x7aa0da });
      mat.polygonOffset = true; mat.polygonOffsetFactor = 1; mat.polygonOffsetUnits = 1; // 避免相鄰內面 z-fighting
      const mesh = new THREE.Mesh(geo, mat);
      mesh.position.set(x - cx, y - cy, z - cz);
      mesh.add(new THREE.LineSegments(new THREE.EdgesGeometry(geo), new THREE.LineBasicMaterial({ color: 0x1b2840 })));
      this.group.add(mesh);
    }
    this.resize();
  }
  resize() {
    const w = Math.min(this.host.clientWidth || 260, 300), h = 200;
    this.renderer.setSize(w, h);
    const aspect = w / h, vs = 5.4;
    this.camera.left = -vs * aspect / 2; this.camera.right = vs * aspect / 2;
    this.camera.top = vs / 2; this.camera.bottom = -vs / 2;
  ...
  start() {
    if (this.raf) return;
    const loop = () => { this.raf = requestAnimationFrame(loop); if (this.host.offsetParent === null) return; this.controls.update(); this.renderer.render(this.scene, this.camera); };
    loop();
  }
  stop() { if (this.raf) { cancelAnimationFrame(this.raf); this.raf = 0; } }
}

/** 全部 88 種形狀：分「面相連(8)」「含邊接觸(80)」兩個 tab 呈現，點縮圖可載入上方 3D 旋轉檢視。 */
function mountShapes(card: HTMLElement) {
  const all = enumerate88();
  const faceC = all.filter(isFaceConnected);
  const edgeC = all.filter((c) => !isFaceConnected(c));
  card.innerHTML = `
    <div class="study-card-h">全部 ${all.length} 種四方塊形狀 <span class="study-sub">面接觸或邊接觸都算相連、鏡像視為不同</span></div>
    <div class="shapes-3d">
      <div class="shapes-3d-host" id="shapes-3d-host"></div>
      <p class="study-note" id="shapes-3d-cap">點下方任一形狀載入這裡，用滑鼠拖曳旋轉看清楚。</p>
    </div>
    <div class="shapes-tabs">
      <button class="shapes-tab active" data-t="face">面相連（${faceC.length}）</button>
      <button class="shapes-tab" data-t="edge">含邊接觸（${edgeC.length}）</button>
    </div>
    <p class="study-note" id="shapes-desc"></p>
    <div class="shapes-grid" id="shapes-grid"></div>`;
  const grid = card.querySelector<HTMLElement>('#shapes-grid')!;
  const desc = card.querySelector<HTMLElement>('#shapes-desc')!;
  const cap = card.querySelector<HTMLElement>('#shapes-3d-cap')!;
  const viewer = new ShapeViewer3D(card.querySelector<HTMLElement>('#shapes-3d-host')!);
  viewer.start();
  const DESC: Record<string, string> = {
    face: '這 8 種是純靠「面」就連起來的方塊，也就是經典的 tetracube——官方 13 個圖形包含了全部這 8 種。',
    edge: '這 80 種至少要靠一條「邊」接觸才連得起來（非面連通）。官方 13 圖形只從中挑了 5 種。紅框那個就是原始套組唯一解不開的「對角雙柱」。',
  };
  const select = (cell: HTMLElement, cells: Cell[], label: string) => {
    grid.querySelectorAll('.shape-cell.sel').forEach((x) => x.classList.remove('sel'));
    cell.classList.add('sel');
    viewer.setShape(cells);
    cap.textContent = `${label}（拖曳旋轉）`;
  };
  const renderTab = (t: 'face' | 'edge') => {
    desc.textContent = DESC[t];
    const list = t === 'face' ? faceC : edgeC;
    const tabName = t === 'face' ? '面相連' : '含邊接觸';
    grid.innerHTML = '';
    list.forEach((cells, i) => {
      ...
      cell.appendChild(cv); cell.appendChild(tag); grid.appendChild(cell);
      drawShapeIso(cv, cells);
      const label = isTwin ? `${tabName} #${i + 1}（對角雙柱）` : `${tabName} #${i + 1}`;
      cell.onclick = () => select(cell, cells, label);
    });
    const first = grid.querySelector<HTMLElement>('.shape-cell');
    if (first) select(first, list[0], `${tabName} #1`);
  };
  card.querySelectorAll<HTMLButtonElement>('.shapes-tab').forEach((b) => {
    b.onclick = () => {
      card.querySelectorAll('.shapes-tab').forEach((x) => x.classList.toggle('active', x === b));
      renderTab(b.dataset.t as 'face' | 'edge');
    };
  });
  renderTab('face');
}
